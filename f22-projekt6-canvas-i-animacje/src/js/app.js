...
class Sky {
    constructor(canvas) {
        ...
        this.nextConstelletion = Math.random() * 3000;
        this.constellation = {
            stars: [],
            isClosed: false,
            width: null,
    ...
    drawStar(star) {
        ...
        for (let i = 0; i < 5; i++) {
            this.ctx.rotate(Math.PI / 180 * (360 / 10)); //w radianach
    ...
    draw(now) {
        ...
        this.drawStars();
        this.updateStars();
        this.drawConstelletion();
        this.updateConstellation();
        this.drawOverlayer();

        if (now - this.lastConstellation > this.nextConstelletion) {
            this.lastConstellation = now;
            this.nextConstelletion = Math.random() * 1000 + 2000;
